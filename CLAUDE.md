...
- `internal/config/`: 対象リポジトリのルート直下の`.masuda/settings.json`（ユーザーが手で編集してコミットする、任意ファイル。`masuda init`が生成する）を読む。`image`（使うDockerイメージ）・`base`（trunk branch名）・`claudeSettings`（`claude`起動時の`--settings`に渡す不透明ペイロード、ADR-0031）・`mcpServers`（子MCPサーバーの宣言、ADR-0043）・`egressAllowlist`（サンドボックスVMのTLS egress許可ホスト名の宣言、Issue #11 M4）の5フィールドを定義する。`resolveImage`/`resolveBase`（`cmd/masuda/main.go`）が`--image`フラグ/`--base`・`--into`フラグ＞`.masuda/settings.json`の値＞デフォルトの優先順位で解決する（`claudeSettings`にCLIフラグでの上書きはない）。`image`フィールドの設計判断（masuda側で言語検出ヒューリスティックを持たずrepo側に委ねる理由）はADR-0015を参照。単一ファイル`.masuda.json`からの再編（破壊的変更、後方互換なし）はADR-0024。`repoRoot/.masuda/settings.local.json`（gitignore対象、`LoadLocal`/`SaveLocal`）は`mcpServers`宣言に対するユーザーの承認・秘密情報、および`egressAllowlist`宣言に対するユーザーの承認を持つ別ファイル（ADR-0043）
...
- `masuda egress list|approve|reject <hostname>`: `.masuda/settings.json`の`egressAllowlist`宣言に対するユーザー承認（`.masuda/settings.local.json`）を管理する（Issue #11 M4）。宣言と承認の両方に含まれるホスト名だけをサンドボックスVMが到達可能（`internal/sandbox.resolveEgressAllowlist`）。`mcpServers`と同じdeclare/approveの分離だが、ホスト名エントリ自体に秘密情報・可変ペイロードが無いため`DeclHash`のようなハッシュ紐付けは無い
