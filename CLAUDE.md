...
**`docs/adr/` は作業前に読むものではない。** 現在何がどう動いているかは、このファイルと `docs/design/` が正となる。ADRを開くのは「なぜこうなっているのか」を問われたとき、または既存の設計判断を覆すときに限り、その場合も番号順に読まず `docs/adr/README.md`（索引）から必要な番号だけを開く。
