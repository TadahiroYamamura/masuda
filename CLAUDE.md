...
- `orchestrator/implement_review_graph.py`: フェーズ4-5（実装・レビュー）の状態遷移ロジック。ADR-0013により1つのオーケストレーターにまとめている。Dockerサンドボックス内で動く
  - フェーズ4: ADR-0009のビルド/テスト自己修正はサブエージェント内で完結させ、オーケストレーターは`implementation_result.json`の結果（done/needs_plan_review/build_test_failed）だけを見る。ADR-0010の機械的バックストップ（PLAN.mdの「変更するファイル一覧」と`git status --porcelain`の突き合わせ、LLM不使用）は実機で逸脱検知を確認済み。逸脱検知時は`DEVIATION.md`を書き出し再オープン（GATE:planとして待機）する
  - **G1再オープンの承認/却下分岐**: 当初`plan_reopened`検知時に即座にゲートマーカー・implementation_result.jsonをクリアしていたが、これだと承認された逸脱も次回チェックで同一の逸脱として検知され無限に再オープンし続けるバグがあった（GATE導入でセッションが自動再開するようになって顕在化）。`.masuda-approved-deviations.json`に承認済み逸脱のファイル一覧を記録し、以後の機械的バックストップから除外する形に修正した。マーカー自体の削除（消費）は`plan_reopened`検知時ではなく、実際に人間の判断（承認/却下）が下された解決時点（`_resolve_plan_reopen`、DEVIATION.mdが既に存在しゲートステータスがpendingでない場合）に行う。却下時・自己申告（needs_plan_review/build_test_failed）の承認時はフェーズ4を再実行する
  - **G1再オープン時の古いゲートマーカー残留バグ（実機で判明、修正済み）**: 上記の「解決時点でのみ削除」という設計により、`plan_reopened`を新規検知した瞬間（DEVIATION.mdがまだ存在しない分岐）ではゲートマーカーに一切手を付けていなかった。ところが最初のG1承認時のマーカー（`.masuda-gate/plan.json`）は`investigate_plan_graph.py`の`detect_phase`が`"approved"`ケースでは削除しない仕様のため、G1承認後は`{"status": "approved", ...}`が消えずに残り続ける。この状態で後から機械的逸脱が検知されると、「`plan.json`のstatusがpendingでなくなるまで待機せよ」という指示（`runtime/CLAUDE.md`）が再オープンした瞬間から既に満たされてしまい、今回の逸脱に対する人間の判断が一度もないまま古い承認を消費しかねない状態になっていた（実機で発生：フェーズ4のClaudeが`inotifywait`を叩く前にマーカーを確認し、この矛盾に自ら気付いて待機した）。`_resolve_plan_reopen`の新規検知分岐で、既存のゲートマーカーがあれば`plan_reopened`を返す前に削除するよう修正し、「pendingでなくなる」が必ず今回のリオープンに対する新しい書き込みだけを指すようにした
...
- `cmd/masuda/`: cobraベースのGo CLI
  ...
  - `masuda plan start <branch> "<task>"`（新規）/ `masuda plan start <workspace-id>`（再開）: 引数が既存ワークスペースIDかどうか（`workspace.Exists`）で新規/再開を判別する。新規はワークスペースID発行＋worktree作成＋フェーズ1-2のホスト側自己ループ起動（`internal/hostloop`。`claude --append-system-prompt-file`でループ仕様を注入、`MASUDA_STATE_DIR`環境変数でオーケストレーターに状態ディレクトリを伝える）。メインセッションは`Bash,Task,Read,Edit(成果物3ファイルのみ、状態ディレクトリの絶対パス)`だけを持ち、実際にリポジトリ内容を読み回る調査・プラン作成は`--agents`で定義したBashなしのカスタムエージェント（`investigator`/`planner`、`Read,Grep,Glob,Edit`のみ）にTask委譲する
    ...
    - **Glob/Grep未許可バグ（実機で判明、修正済み）**: `allowedTools()`は当初`Read`だけをパス制限なしで事前承認しており、`Grep`・`Glob`は一切リストに含まれていなかった。investigator/planner両エージェントは元々エージェントレベルではGlob/Grepツールを持っている（`customAgentsJSON`のTools一覧）が、セッションレベルの許可ルールに乗っていなかったため、外部状態ディレクトリ（roadmap step 7でworktree外に出た）へのGlob呼び出しがClaude Codeのデフォルト確認プロンプトに落ち、実機でplannerが`Search(...)`の未回答プロンプトで停止する事態が発生した。`Read`と同じ理由（repo側の`permissions.deny`は引き続き効く）で`Grep,Glob`もパス制限なしで許可リストに追加して解消した
    - **repoRootの取り違えバグ（実機で判明、go:embedで修正済み）**: `internal/hostloop.Start`は当初、対象リポジトリの`repoRoot()`（cwdの`git rev-parse --show-toplevel`）を使って`venv/bin/python`・`orchestrator/investigate_plan_graph.py`のパスを組み立てていた。これはmasuda自身のリポジトリを対象に自己回帰的にテストしていた間は「対象リポジトリ＝masuda自身のチェックアウト」で偶然一致していたため露呈しなかったが、外部の対象リポジトリ（`oncall_pf_template`）に対して使ったところ壊れたパスが生成され、ホストループ内のAIが自力で本物のオーケストレーターの場所を突き止めて回避する事態になった。`internal/sandbox.Start`の`runtime/CLAUDE.md`パス（`docker cp`の焼き込み元）も同じ取り違えをしていた。修正として、`assets.go`（リポジトリルート直下、`go:embed`のパターンは宣言ファイル自身のディレクトリ以下にしか到達できず`..`を挟めないため、`orchestrator/`・`runtime/`と同じ階層に置く必要がある）でこれらをGoバイナリに焼き込み、`internal/hostloop/bootstrap.go`がXDGベースの固定ディレクトリ（`<DataHome>/masuda/runtime/`）へ抽出・venv自己構築（`requirements.txt`のsha256マーカーで再構築要否を判定）するようにした。**開発時のはまりどころ**: `orchestrator/investigate_plan_graph.py`や`runtime/CLAUDE.md`を編集した後、`go build`し直さないと埋め込み内容が更新されない（Dockerイメージの再ビルド忘れ、と対になる注意点）
  - `masuda chat <workspace-id>`: 元は`plan chat`/`review chat`にゲートごと分かれていたが、attach先（フェーズ1-2ホストループかフェーズ3-5サンドボックスか）はゲート名ではなくどちらのセッションが生きているかだけで決まる（review側の分岐はplan側の分岐の単なる部分集合だった）ため、ゲート名を引数に取らない単一のトップレベルコマンドに統合した。`internal/hostloop.AttachArgs`（ホスト、`tmux attach`）を先に試し、なければ`internal/sandbox.AttachArgs`（Docker、`docker exec -it ... tmux attach`）を試す——G1がフェーズ1-2ホストループ・フェーズ4-5のDocker再オープンどちらで待機していても対応する。実機でfast-path（`plan/review approve`）・chatパス（対話でClaude自身がマーカーを書く）の両方をG1・G2で確認済み
