...
- 一部改訂: [[0061-daemon-liveness-by-socket-pid-only-after-proc-identification]] — Decisionが挙げる「PIDファイル＋signal 0による冪等な起動」は、生存判定がtrustedソケットへのdialに変わり、PIDは`/proc/<pid>/cmdline`で同定してから停止に使うだけになった。起動する入口もゲート系コマンドと`sandbox start`へ広がった
