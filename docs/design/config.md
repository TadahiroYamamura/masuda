...
- **`image`**（string）: `masuda sandbox start`・`masuda review start`等サンドボックスを起動するコマンドが使う**イメージエントリ名**（`.masuda/images/<entry>/`、ADR-0054）。Dockerのタグ名ではない——ローカルタグはエントリ名から導出される。エントリの中身とビルドは`docs/design/images-and-rootfs.md`を参照
...
- **`privilegedCommands`**（`map[string]PrivilegedCommandDecl`）: rootやDockerデーモンを必要とするコマンドの宣言（`{Command, Image, TimeoutSeconds, Outputs}`）。`mcpServers`と同じdeclare/approve分離で、承認は`settings.local.json`側にある。ハッシュ固定の対象が宣言だけでない（イメージエントリの内容も含む）点を含め、機構全体は`docs/design/privileged-commands.md`を参照
